...
class JwtFacades 
{
    private static $expires = 600;                   //设置单次过期时间(分钟)
    private static $maxExpires = 600 * 24 * 30;      //最大过期时间(分钟),app使用
    ...
    {
        //尝试获取token
        $token = self::tryToGetToken($request, $token);
        if (!$token) {
            return false;
        }
        ...
        if (self::$type == 'token') {
            ...
            \Cache::put(self::$sessionPrefix . ':' . self::$seesionKey, $data, $expires_time);
        } else {
     ...
     * @return bool
     */
    protected static function tryToGetToken($request, $token)
    {

        if (empty($token)) {
            //获取header内容
            $token = self::getTokenByHeader();
        }
        if (empty($token)) {
            //获取url上token字段上的内容
            $token = self::getTokenByUrl();
        }

        if (empty($token)) {
            //获取cookie上的内容
            $token = self::getTokenByCookie();
            self::$type = 'session';
    ...
    public static function createToken(array $session_data = array(), $device = '')
    {
        ...
        \Cache::put(self::$sessionPrefix . ':' . $session_key, $session_data, $session_expires_time);
        return [
            'token' => JWT::encode($data, config('app.key') ,'HS256' ,'hiker'),
            'sk' => $secret_key,
     ...
     * @param $minute
     */
    public static function setExpires($minute)
    {
        self::$expires = $minute;
    }
     ...
     * @return array
     */
    public static function routeInit()
    {
