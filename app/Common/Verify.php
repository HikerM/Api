...
class Verify
{
    ...
    private $multi_array_data = array();
    ...
    public function getData()
    {
        ...
        if (!empty($this->multi_array_data)) {
            foreach ($this->multi_array_data as $array_key => $array_data) {
                $this->data = Arr::add($this->data, $array_key, []);
            }
        }
        ...
        foreach ($rule as $field => $v) {
            ...
            if (count($field_array) > 1) {        //数组
                ...
                if ($field_array[1] !== '*') {    //二维数组
                    ...
                    if (isset($data[$field_array[0]]) && !isset($this->temData[$field_array[0]])) {
                        $this->multi_array_data[$field_array[0]] = [];
                    }
                    ...
                    //不是数组
                    if (isset($data[$field_array[0]]) && !is_array($data[$field_array[0]])) {
                        throw new ApiException($field . '验证字段不存在！', $field . '_ERROR', 422);
                    }
    ...
    {
        //判断是否可以空,仅限空字符串
        if (in_array('can_null', $vail_type) && ($vail_data === '' || $vail_data === null)) {
            return true;
    ...
    {
        if (!(preg_match('/^\d{10}$/', $time) || preg_match('/^\d{13}$/', $time))) {
            return false;
        }
